...
public class IndexMasterObserver extends BaseMasterObserver {
	...
	public void preModifyTableHandler(
		...
		HTableDescriptor oldDesc = ctx.getEnvironment().getMasterServices()
		...
		if (oldDesc.getValue(IndexLoadBalancer.PARENT_TABLE_KEY) == null
			...
			TableName userTableName = TableName.valueOf(htd
					.getValue(IndexLoadBalancer.PARENT_TABLE_KEY));
	...
	public void postMove(ObserverContext<MasterCoprocessorEnvironment> ctx,
		...
		if (balancer != null && balancer.isTableColocated(region.getTable())) {
			AssignmentManager am = ctx.getEnvironment().getMasterServices()
					.getAssignmentManager();
			RegionStates regionStates = am.getRegionStates();
			String tableName = region.getTable().getNameAsString();
			String correspondingTable = region.getTable().getNameAsString()
			...
			List<HRegionInfo> regions = regionStates
					.getRegionsOfTable(TableName.valueOf(correspondingTable));
