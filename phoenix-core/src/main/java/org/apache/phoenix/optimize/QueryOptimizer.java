        ...
        if (PIndexState.ACTIVE.equals(resolver.getTables().get(0).getTable().getIndexState())) {
            try {
                QueryCompiler compiler =
                        new QueryCompiler(statement, indexSelect, resolver, targetColumns,
                ...
                // local index. It's like full scan of both local index and data table which is
                // inefficient.
                // Then we don't use the index. If all the columns to project are present in the
                // index
                ...
                // Checking number of columns handles the wildcard cases correctly, as in that case
                // the index
