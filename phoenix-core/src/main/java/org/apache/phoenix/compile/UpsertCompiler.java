...
public class UpsertCompiler {
            ...
            if (SchemaUtil.isPKColumn(column)) {
                ...
                if (SchemaUtil.getPKPosition(table, column) == table.getRowTimestampColPos()) {
                    if (!useServerTimestamp) {
                        ...
                        rowTimestamp =
                                PLong.INSTANCE.getCodec().decodeLong(value, 0,
                                    rowTimestampCol.getSortOrder());
                        if (rowTimestamp < 0) {
    ...
    private static MutationState upsertSelect(PhoenixStatement statement, 
            ...
            int[] pkSlotIndexes, boolean useServerTimestamp) throws SQLException {
        try {
    ...
    private static class UpsertingParallelIteratorFactory extends MutatingParallelIteratorFactory {
        ...
        protected MutationState mutate(StatementContext context, ResultIterator iterator, PhoenixConnection connection) throws SQLException {
            ...
            return upsertSelect(statement, tableRef, projector.cloneIfNecessary(), iterator, columnIndexes, pkSlotIndexes,useSeverTimestamp);
        }
    ...
    public MutationPlan compile(UpsertStatement upsert) throws SQLException {
            ...
            return new MutationPlan() {
                ...
                public MutationState execute() throws SQLException {
                    ...
                    if (parallelIteratorFactory == null) {
                        return upsertSelect(statement, tableRef, projector, iterator, columnIndexes, pkSlotIndexes,useServerTimestamp);
                    }
                    try {
        ...
        return new MutationPlan() {
            ...
            public MutationState execute() throws SQLException {
                ...
                setValues(values, pkSlotIndexes, columnIndexes, tableRef.getTable(), mutation, statement,useServerTimestamp);
