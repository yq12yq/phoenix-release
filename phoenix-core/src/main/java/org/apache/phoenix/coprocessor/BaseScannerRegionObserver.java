    ...
    protected RegionScanner getWrappedScanner(final ObserverContext<RegionCoprocessorEnvironment> c,
        ...
        return new RegionScanner() {
            ...
            public NextState next(List<Cell> result, int limit, long remainingResultSize)
                throws IOException {
                try {
                    return s.next(result, limit, remainingResultSize);
                } catch (Throwable t) {
                    ServerUtil.throwIOException(c.getEnvironment().getRegion().getRegionNameAsString(), t);
                    return null; // impossible
                }
            ...
            public NextState nextRaw(List<Cell> result) throws IOException {
                try {
                    NextState next = s.nextRaw(result);
                    if (result.size() == 0) {
                        return next;
                    }
            ...
            public NextState nextRaw(List<Cell> result, int limit, long remainingResultSize)
                throws IOException {
              try {
                NextState next = s.nextRaw(result, limit, remainingResultSize);
                if (result.size() == 0) {
                    return next;
                }
                if (arrayFuncRefs != null && arrayFuncRefs.length > 0 && arrayKVRefs.size() > 0) {
