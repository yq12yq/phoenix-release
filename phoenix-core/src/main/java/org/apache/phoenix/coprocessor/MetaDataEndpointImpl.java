    ...
    private MetaDataMutationResult doDropTable(byte[] key, byte[] tenantId, byte[] schemaName,
        ...
        try {
            ...
            if (tableViewFinderResult.hasViews()) {
                if (isCascade) {
                    if (tableViewFinderResult.allViewsInMultipleRegions()) {
                        ...
                        // Recursively delete views - safe as all the views as
                        ...
                        for (Result viewResult : tableViewFinderResult.getResults()) {
                            ...
                            MetaDataMutationResult result =
                                    doDropTable(viewKey, viewTenantId, viewSchemaName, viewName,
                                        null, PTableType.VIEW, rowsToDelete, invalidateList, locks,
            ...
            do {
                ...
                int nColumns =
                        getVarChars(kv.getRowArray(), kv.getRowOffset(), kv.getRowLength(), 0,
                            rowKeyMetaData);
                if (nColumns == 5
                ...
                // args was introduced
                // in 0.94.4, thus if we try to use it here we can no longer use
                // the 0.94.2 version
                // of the client.
                Delete delete =
                        new Delete(kv.getRowArray(), kv.getRowOffset(), kv.getRowLength(),
                                clientTimeStamp);
                rowsToDelete.add(delete);
                results.clear();
                scanner.next(results);
